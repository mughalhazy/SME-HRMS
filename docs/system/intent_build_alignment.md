...
| P3 | attendance service (`attendance_service/*`) | `/api/v1/attendance/*` | `002_workflow_schema.sql` (`attendance_records`) + `003_persistence_normalization.sql` ext | `tests/test_attendance_service.py`, `tests/test_attendance_ui.py` | COMPLETE |
...
| P9 | event contract normalization (`event_contract.py`) | event payload contract (D2) | `005_event_outbox.sql` / outbox persistence linkage | `tests/test_outbox_system.py`, `tests/test_backward_compatibility_enforcement.py` | COMPLETE |
...
| P11 | tenant foundation (`tenant_support.py`, gateway tenant middleware) | tenant-scoped `/api/v1/*` | `004_tenant_foundation.sql` | `tests/test_gateway_tenant_context.py`, `tests/test_security_compliance_lock.py` | COMPLETE |
| P12 | audit service (`audit_service/*`) | `/api/v1/audit/*` | `005_audit_service.sql` (`audit_records`) | `tests/test_audit_service.py`, `tests/test_security_logging.py` | PARTIAL (service not deployed in compose) |
| P13 | auth service (`auth` models in migrations + service API surface) | `/api/v1/auth/*` | `003_persistence_normalization.sql` (`user_accounts`, `role_bindings`, `permission_policies`, `sessions`, `refresh_tokens`) | `tests/test_auth_service.py` | COMPLETE |
| P14 | notification service (`notification_service.py`, `notification_api.py`) | `/api/v1/notifications/*` | `005_notification_service.sql` | `tests/test_notification_service.py` | COMPLETE |
| P15 | background job system (`background_jobs.py`, `background_jobs_api.py`) | `/api/v1/jobs/*` (gateway-local job route) | `005_background_jobs_schema.sql` (`background_jobs`, failures) | `tests/test_background_jobs.py`, `tests/test_gateway_load_control.py` | PARTIAL (compose wiring is indirect/in-process) |
| P16 | outbox system (`outbox_system.py`, `event_outbox.py`) | internal dispatch APIs/events | `005_event_outbox.sql` + `005_background_jobs_schema.sql` (`event_outbox`) | `tests/test_outbox_system.py` | COMPLETE |
...
| P25 | engagement service (`engagement_service.py`, `engagement_api.py`) | `/api/v1/engagement/*` | `006_engagement_service.sql` | `tests/test_engagement_service.py` | PARTIAL (not in gateway route registry) |
...
| P30 | event reliability pass | outbox/replay stability | outbox tables (`005_event_outbox.sql`, `005_background_jobs_schema.sql`) | `tests/test_outbox_system.py`, resilience tests | COMPLETE |
