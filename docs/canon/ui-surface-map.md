...
This document maps every UI surface to canonical read models, capabilities, and service owners.

## Mapping table

| UI Surface | Primary Read Model(s) | Capability ID(s) | Primary Service Owner | Domain Entities |
|---|---|---|---|---|
| `dashboard` | `employee_directory_view`, `attendance_dashboard_view`, `leave_requests_view`, `payroll_summary_view`, `candidate_pipeline_view`, `performance_review_view` | `CAP-EMP-001`, `CAP-ATT-001`, `CAP-LEV-001`, `CAP-PAY-001`, `CAP-HIR-001`, `CAP-PRF-001` | `employee-service` | Employee, AttendanceRecord, LeaveRequest, PayrollRecord, JobPosting, Candidate, PerformanceReview |
...
| `job_postings` | `candidate_pipeline_view` | `CAP-HIR-001` | `hiring-service` | JobPosting, Department, Role |
...
## Notes
- Each UI surface has at least one mapped read model, one capability, and one owning service.
- Cross-module widgets on `dashboard` are composed from multiple read models but use stable capability IDs.
