...
Created: 2026-06-17
Supersedes: `DECISION_ESCALATION_MATRIX.md` (2026-06-15 revision)
Owner: Human
...
This revision introduces `SAFE_REPOSITORY_HYGIENE` as **TIER 1.5** between AUTONOMOUS and REQUIRES_APPROVAL. All other tiers are unchanged. See `APPROVAL_RECLASSIFICATION_REPORT.md` for the reclassification of all open items under the new tier. See `SAFE_REPOSITORY_HYGIENE_POLICY.md` for full policy definition and execution guidelines reference.

The original `DECISION_ESCALATION_MATRIX.md` has been updated in-place to match this revision. This file serves as the standalone revised version for the governance refinement report package.
...
Full policy and qualifying/disqualifying criteria: `SAFE_REPOSITORY_HYGIENE_POLICY.md`
...
- Relocating reports to `docs/08_reports/`
- Fixing broken cross-references in documentation
- Correcting metadata (Status field, Last Reviewed date, phase identifier)
- Adding missing document headers or status blocks

### Archive Maintenance
- Moving confirmed-dead source files to archive directories (e.g., `backend/docs/system/archive/`)
- "Confirmed-dead" = zero active import callers confirmed by grep, superseded by replacement
- Note: archive (move to archive directory) only — deletion of files outside `docs/` is TIER 2

### Folder Restructuring (safe subset)
- Creating new organizational subdirectories under `docs/` or archive locations
- Renaming `docs/` subdirectories where no build tool, import, or external config references the path
...
- Marking superseded fragments with updated Status fields

### Generated & Temporary Artifact Cleanup
- Removing OS-generated files (`.lnk`, `.DS_Store`, `Thumbs.db`) from the repository
- Removing auto-generated files committed by mistake (`.pyc`, `__pycache__` entries)
- Removing scratch or temp files from repository root

### Root-Level Cleanup
- Removing non-project files from repository root
- Creating `README.md` or `CONTRIBUTING.md` if absent

### .gitignore Improvements
- Adding new ignore patterns (additive only)
- Reorganizing `.gitignore` sections with comment headers
- Never removing existing patterns (that is TIER 2)

### Governance Metadata Updates
- Updating `Last Reviewed` dates and `Status` fields
- Adding cross-references between governance documents
- Creating new governance registers or classification matrices

### Archiving Dead CI Workflow Files
- Moving never-executing CI workflow files (e.g., from `backend/.github/workflows/`) to archive directories for reference
- Scope: file-level move only, no YAML content modification, no activation in root `.github/workflows/`
...
- Modifying CI/CD pipeline (`.github/workflows/ci.yml`) or activating inactive workflows
...
### Frontend Relocation
- Moving the active Next.js frontend application directory (`backend/ui/`) to any new location
- Any docker-compose.yml context or path change for the frontend-ui service
...
| Create `APPROVAL_RECLASSIFICATION_REPORT.md` in `docs/08_reports/` | SAFE_REPOSITORY_HYGIENE | Report generation |
| Update `Last Reviewed` date in governance document | SAFE_REPOSITORY_HYGIENE | Governance metadata update |
| Remove `.DS_Store` file committed to root | SAFE_REPOSITORY_HYGIENE | Temporary artifact cleanup |
...
| Migrate `build.yml` content + activate in root CI | REQUIRES APPROVAL | CI/CD activation + content modification |
...
When an action falls into SAFE_REPOSITORY_HYGIENE:

1. **State the action** — one sentence describing what will happen and why
2. **Execute** — proceed without waiting for approval
3. **Record** — document in the relevant register
4. **Report** — include in phase summary

Example REQUIRES APPROVAL:
> "I intend to add a new column `middle_name TEXT` to the `employees` table. This requires a new migration file (`016_employee_middle_name.sql`). This will affect the employee-service and any frontend components that display employee names. This change is not reversible without another migration. Shall I proceed?"

Example SAFE_REPOSITORY_HYGIENE:
> "Moving 43 confirmed-dead TypeScript employee-service files to `backend/docs/system/archive/typescript-employee-service/` — these have zero Python callers (confirmed by grep). No production code is affected."
