...
import { AlertTriangle, ArrowUpRight, BadgeDollarSign, Building2, CheckCircle2, Clock3, CreditCard, Eye, MoreHorizontal, RefreshCw, ShieldAlert, Wallet } from 'lucide-react'
...
const kpiCards = [
  {
    label: 'Current Cycle',
    value: 'March 2026',
    description: 'Primary payroll window',
    icon: Clock3,
  ...
  },
  {
    ...
    value: '1180',
    description: 'Successfully queued and settled',
    icon: CheckCircle2,
  ...
  },
  {
    label: 'Total Payroll',
    value: '$1,240,000',
    description: 'Gross amount for this cycle',
    icon: Wallet,
...
const exceptions = [
  {
    ...
    value: '12',
    detail: 'Needs payment gateway review',
    icon: CreditCard,
  ...
  },
  {
    ...
    value: '23',
    detail: 'Employees cannot be disbursed',
    icon: Building2,
...
export function Payroll() {
  return (
    <div className="min-h-full bg-[#f7f8fc] px-6 py-6 text-slate-900">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-semibold tracking-tight text-slate-950">Payroll</h1>
            <p className="text-sm text-slate-500">Monitor cycles, track exceptions, and manage disbursement runs.</p>
          </div>
                      ...
                      return (
                        <TableRow key={run.runId} className="border-slate-100 hover:bg-slate-50/80">
                          <TableCell className="px-5 py-3.5">
                            <div className="font-semibold text-slate-800">{run.runId}</div>
                          </TableCell>
                          ...
                          <TableCell className="py-3.5 pr-5 text-right">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button className="h-8 w-8 rounded-lg" size="icon" type="button" variant="ghost">
                              ...
                              <DropdownMenuContent align="end" className="border-slate-200 bg-white">
                                ...
                                <DropdownMenuItem className={cn(run.status !== 'Failed' && 'text-slate-400')} disabled={run.status !== 'Failed'}>
          ...
          <div className="space-y-5">
            ...
            <Card className="border-slate-200 bg-white shadow-sm">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-sm text-slate-500">Latest payroll processed logs and review trail.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  <div key={`${item.title}-${item.time}`}>
                    <div className="flex gap-3">
                      <div className="flex flex-col items-center">
                        ...
                        {index < recentActivity.length - 1 ? <Separator className="mt-2 h-10" orientation="vertical" /> : null}
                      </div>
                      <div className="min-w-0 flex-1 pb-1">
                        <p className="text-sm font-medium leading-5 text-slate-800">{item.title}</p>
                        <div className="mt-1 flex items-center gap-1 text-xs font-medium uppercase tracking-[0.12em] text-slate-400">
                          <span>{item.time}</span>
