      ...
      <PageHero
        ...
        description={
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            ...
            <p className="text-slate-500">Compare review progress, spotlight standout employees, and move directly into decisions.</p>
          </div>
        }
        actions={
          <>
        ...
        <div className="overflow-hidden rounded-[var(--radius-surface)] border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-surface)]">
          <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">A list-first workspace that keeps status, due date, and rating context visible without falling into a dense table.</p>
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 sm:w-[220px]">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Queue focus</p>
          ...
          <div className="divide-y divide-slate-200">
            {reviewQueue.map((review) => (
              ...
              >
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(260px,0.9fr)_24px] xl:items-center">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3 xl:gap-2">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                      ...
                      <p className="mt-2 text-sm font-semibold text-slate-950">{review.rating ?? '—'}</p>
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Due date</p>
                      <p className="mt-2 text-sm font-semibold text-slate-950">{review.dueDate}</p>
                    </div>
                  </div>
          ...
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="space-y-4">
              {insightGroups.map((group) => {
                ...
                return (
                  <div key={group.title} className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <div className={`flex items-center gap-2 ${group.tone}`}>
                      <Icon className="h-4 w-4" />
                      <p className="text-xs font-semibold uppercase tracking-[0.18em]">{group.title}</p>
                    </div>
                    <div className="space-y-3">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.8fr)]">
        <Card className="border-slate-200 bg-white shadow-sm">
          <CardHeader className="border-b border-slate-200 pb-4">
            ...
            <CardTitle className="text-base">Past review cycles</CardTitle>
            <p className="text-sm text-slate-600">Recent summaries keep calibration history close to the active queue.</p>
          </CardHeader>
