...
# ---------------------------------------------------------------------------
# GET /api/v1/analytics/anomalies/insights
# G21 — InsightEngine wired to reporting-analytics anomaly signal feed
# Generates explainable HR insights (attrition risk, overtime trends, payroll anomalies)
# Signals with score >= 40 are forwarded to decision-service as AnomalySignalEmitted events.
# ---------------------------------------------------------------------------
...
def get_anomaly_insights(
    ...
    """
    Generate explainable HR insights via InsightEngine.
    Accepts: { attrition: {...}, overtime: {...}, payroll: {...} }
    Returns insight signals with score, confidence, explanation.
    High/medium signals (score >= 40) should be emitted as AnomalySignalEmitted events
    consumed by decision-service.
    """
    insights = _insight_engine.generate(payload)
    signals = []
    for insight_type, insight in insights.items():
        ...
        if score >= 40:
            signals.append({
                ...
                "score": score,
                "threshold_level": "high" if score >= 70 else "medium",
                **insight,
            })
    return 200, {
        "insights": insights,
