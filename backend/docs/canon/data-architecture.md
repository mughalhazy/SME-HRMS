...
| source | VARCHAR(20) | Yes |  | `Manual`, `Biometric`, `APIImport`, `GEO_FENCE`, `FACE_RECOGNITION`, `MOBILE`. |
...
## Compensation domain tables

### Table: `compensation_bands`

| Column | Type | Null | Key | Constraints / Notes |
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No |  | Tenant partition key. |
| compensation_band_id | UUID | No | PK | Primary identifier. |
| grade_band_id | UUID | No | FK | References `grade_bands.grade_band_id`. |
| name | VARCHAR(150) | No |  | Band display name. |
| code | VARCHAR(30) | No | UQ | Unique code per tenant. |
| currency | CHAR(3) | No |  | ISO currency code. |
| min_salary | NUMERIC(12,2) | No |  | Minimum salary; `>= 0`. |
| max_salary | NUMERIC(12,2) | No |  | Maximum salary; `>= min_salary`. |
| target_salary | NUMERIC(12,2) | Yes |  | Optional midpoint target. |
| status | VARCHAR(20) | No |  | `Draft`, `Active`, `Inactive`, `Archived`. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
| updated_at | TIMESTAMPTZ | No |  | Last update timestamp. |

**Indexes**
- `pk_compensation_bands (compensation_band_id)`
...
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No |  | Tenant partition key. |
| salary_revision_id | UUID | No | PK | Primary identifier. |
| employee_id | UUID | No | FK | References `employees.employee_id`. |
| compensation_band_id | UUID | Yes | FK | References `compensation_bands.compensation_band_id`. |
| effective_from | DATE | No |  | Effective start date. |
| effective_to | DATE | Yes |  | Effective end date; null = indefinite. |
| base_salary | NUMERIC(12,2) | No |  | Base salary amount; `>= 0`. |
| currency | CHAR(3) | No |  | ISO currency code. |
| reason | TEXT | Yes |  | Freeform revision reason. |
| status | VARCHAR(20) | No |  | `Draft`, `Approved`, `Superseded`. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
| updated_at | TIMESTAMPTZ | No |  | Last update timestamp. |

**Indexes**
- `pk_salary_revisions (salary_revision_id)`
...
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No |  | Tenant partition key. |
| benefits_plan_id | UUID | No | PK | Primary identifier. |
| code | VARCHAR(30) | No | UQ | Unique code per tenant. |
| name | VARCHAR(150) | No |  | Plan display name. |
| plan_type | VARCHAR(50) | No |  | Plan category. |
| employer_contribution | NUMERIC(12,2) | No |  | Default employer contribution. |
| employee_contribution | NUMERIC(12,2) | No |  | Default employee contribution. |
| currency | CHAR(3) | No |  | ISO currency code; default `PKR`. |
| status | VARCHAR(20) | No |  | `Draft`, `Active`, `Inactive`, `Archived`. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
| updated_at | TIMESTAMPTZ | No |  | Last update timestamp. |

**Indexes**
- `pk_benefits_plans (benefits_plan_id)`
...
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No |  | Tenant partition key. |
| benefits_enrollment_id | UUID | No | PK | Primary identifier. |
| employee_id | UUID | No | FK | References `employees.employee_id`. |
| benefits_plan_id | UUID | No | FK | References `benefits_plans.benefits_plan_id`. |
| employee_contribution | NUMERIC(12,2) | No |  | Employee-elected contribution amount. |
| employer_contribution | NUMERIC(12,2) | No |  | Employer contribution amount. |
| effective_from | DATE | No |  | Enrollment effective start date. |
| effective_to | DATE | Yes |  | Enrollment effective end date; null = indefinite. |
| status | VARCHAR(20) | No |  | `Draft`, `Active`, `Cancelled`, `Expired`. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
| updated_at | TIMESTAMPTZ | No |  | Last update timestamp. |

**Indexes**
- `pk_benefits_enrollments (benefits_enrollment_id)`
...
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No |  | Tenant partition key. |
| allowance_id | UUID | No | PK | Primary identifier. |
| employee_id | UUID | No | FK | References `employees.employee_id`. |
| code | VARCHAR(60) | No |  | Allowance type code. |
| amount | NUMERIC(12,2) | No |  | Allowance amount; `>= 0`. |
| currency | CHAR(3) | No |  | ISO currency code; default `PKR`. |
| frequency | VARCHAR(20) | No |  | `Monthly`, `OneTime`, `Quarterly`, `Annual`. |
| effective_from | DATE | No |  | Effective start date. |
| effective_to | DATE | Yes |  | Effective end date; null = indefinite. |
| status | VARCHAR(20) | No |  | `Draft`, `Active`, `Inactive`, `Archived`. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
...
| Column | Type | Null | Key | Constraints / Notes |
|---|---|---:|---|---|
| tenant_id | VARCHAR(80) | No | FK |  Tenant partition key. |
| travel_request_id | UUID | No | PK | Primary identifier. |
| employee_id | UUID | No | FK | References `employees.employee_id`. |
| manager_employee_id | UUID | No | FK | References `employees.employee_id`. |
| purpose | TEXT | No |  | Business purpose of travel. |
| trip_type | VARCHAR(20) | No |  | `OneWay`, `RoundTrip`, `MultiCity`. |
| origin_city | VARCHAR(100) | No |  | Travel origin. |
| destination_city | VARCHAR(100) | No |  | Primary destination. |
| start_date | DATE | No |  | Requested start date. |
| end_date | DATE | No |  | Requested end date. |
| estimated_cost | NUMERIC(12,2) | No |  | Estimated trip cost. |
| currency | CHAR(3) | No |  | ISO currency code. |
| status | VARCHAR(20) | No |  | `Draft`, `Submitted`, `Approved`, `Booked`, `Completed`, `Rejected`, `Cancelled`. |
| workflow_id | UUID | Yes | FK | References centralized workflow instance. |
| submitted_at | TIMESTAMPTZ | Yes |  | Submit timestamp. |
| decision_at | TIMESTAMPTZ | Yes |  | Final decision timestamp. |
| approved_at | TIMESTAMPTZ | Yes |  | Approval timestamp. |
| booked_at | TIMESTAMPTZ | Yes |  | Booking completion timestamp. |
| completed_at | TIMESTAMPTZ | Yes |  | Travel completion timestamp. |
...
**Indexes**
- `pk_travel_requests (travel_request_id)`
- `idx_travel_requests_employee_id (employee_id)`
- `idx_travel_requests_status (status)`
...
| segment_type | VARCHAR(20) | No |  | `Flight`, `Rail`, `Hotel`, `Car`, `Other`. |
| departure_city | VARCHAR(100) | No |  | Segment origin. |
| arrival_city | VARCHAR(100) | No |  | Segment destination. |
| departure_at | TIMESTAMPTZ | No |  | Scheduled departure. |
| arrival_at | TIMESTAMPTZ | No |  | Scheduled arrival. |
| provider_name | VARCHAR(150) | Yes |  | Airline, hotel, rail, or vendor name. |
| booking_reference | VARCHAR(80) | Yes |  | Booking or ticket reference. |
| lodging_name | VARCHAR(150) | Yes |  | Hotel or lodging name when applicable. |
| lodging_address | TEXT | Yes |  | Lodging address when applicable. |
| notes | TEXT | Yes |  | Segment-specific notes. |
| created_at | TIMESTAMPTZ | No |  | Creation timestamp. |
| updated_at | TIMESTAMPTZ | No |  | Last mutation timestamp. |

**Indexes**
- `pk_travel_itinerary_segments (segment_id)`
- `idx_travel_segments_travel_request_id (travel_request_id)`
...
- `compensation_bands.grade_band_id -> grade_bands.grade_band_id`
- `salary_revisions.employee_id -> employees.employee_id`
- `salary_revisions.compensation_band_id -> compensation_bands.compensation_band_id`
- `benefits_enrollments.employee_id -> employees.employee_id`
- `benefits_enrollments.benefits_plan_id -> benefits_plans.benefits_plan_id`
- `allowances.employee_id -> employees.employee_id`
- `travel_requests.employee_id -> employees.employee_id`
- `travel_requests.manager_employee_id -> employees.employee_id`
- `travel_itinerary_segments.travel_request_id -> travel_requests.travel_request_id`
