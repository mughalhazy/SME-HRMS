...
| Attribute | Type | Required | Notes |
|---|---|---|---|
| benefits_plan_id | UUID | Yes | Primary identifier. |
| tenant_id | String | Yes | Tenant partition key. |
| code | String | Yes | Unique plan code per tenant. |
| name | String | Yes | Plan display name. |
| plan_type | String | Yes | Plan category. |
| employer_contribution | Decimal(12,2) | Yes | Default employer contribution amount. |
| employee_contribution | Decimal(12,2) | Yes | Default employee contribution amount. |
| currency | String | Yes | ISO 4217 currency code; default `PKR`. |
| status | Enum | Yes | `Draft`, `Active`, `Inactive`, `Archived`. |
| created_at | DateTime | Yes | Record creation timestamp. |
| updated_at | DateTime | Yes | Record last update timestamp. |
...
| Attribute | Type | Required | Notes |
|---|---|---|---|
| benefits_enrollment_id | UUID | Yes | Primary identifier. |
| tenant_id | String | Yes | Tenant partition key. |
| employee_id | UUID | Yes | Foreign key to `Employee`. |
| benefits_plan_id | UUID | Yes | Foreign key to `BenefitsPlan`. |
| employee_contribution | Decimal(12,2) | Yes | Employee-elected contribution amount. |
| employer_contribution | Decimal(12,2) | Yes | Employer contribution amount. |
| effective_from | Date | Yes | Enrollment effective start date. |
| effective_to | Date | No | Enrollment effective end date; null = indefinite. |
...
| Attribute | Type | Required | Notes |
|---|---|---|---|
| allowance_id | UUID | Yes | Primary identifier. |
| tenant_id | String | Yes | Tenant partition key. |
| employee_id | UUID | Yes | Foreign key to `Employee`. |
| code | String | Yes | Allowance type code. |
| amount | Decimal(12,2) | Yes | Allowance amount; `>= 0`. |
| currency | String | Yes | ISO currency code; default `PKR`. |
| frequency | Enum | Yes | `Monthly`, `OneTime`, `Quarterly`, `Annual`. |
| effective_from | Date | Yes | Effective start date. |
| effective_to | Date | No | Effective end date; null = indefinite. |
| status | Enum | Yes | `Draft`, `Active`, `Inactive`, `Archived`. |
| created_at | DateTime | Yes | Record creation timestamp. |
| updated_at | DateTime | Yes | Record last update timestamp. |
...
| source | Enum | No | `Manual`, `Biometric`, `APIImport`, `GEO_FENCE`, `FACE_RECOGNITION`, `MOBILE`. |
