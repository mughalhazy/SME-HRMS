...
> NOTE (2026-06-13): `JobPostingOnHold`, `InterviewCancelled`, and `InterviewNoShow` were added above for parity with `docs/canon/service-map.md`'s `hiring-service` Publishes list (B/C-category fix). `CandidateStageTransitionRecorded` is retained as a hiring-service-local addition not present in service-map.md's list — left as-is per [[feedback_divergence_resolution]] (extension, not deletion).
