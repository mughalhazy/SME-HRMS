...
> **⚠️ VERIFICATION SCOPE NOTE:** The formal "FULLY ALIGNED (verified)" declaration below is backed by evidence from **2026-03-31 only** (pre-Session 2: 281 pytest, QC 11/11, RE-QC all green). Sessions 2–8 added 50+ new service files; their changes are appended below as narrative update sections but **no QC re-run has been executed** covering those sessions. Per `docs/system/catalogue.md` §9 rule: this file must not be updated speculatively — only after a full clean QC suite run. The alignment claim is therefore valid for the 2026-03-31 codebase snapshot only. `docs/system/progress.md` is the current gap-closure state of record (last updated Session 8, 2026-04-14).
...
## Session 2 — Service layer completion update (2026-04-12)

All backend service gaps G01–G29 resolved. New files added to the build:

- `core/country_resolver.py` — made data-driven. `register_adapter()` / `register_mapping()` / `seed_dev_defaults()`. Pakistan moved out of `__init__`.
- `services/compliance_service.py` — full rewrite (~200 lines). Country-agnostic. DRAFT→VALIDATED→SUBMITTED→ACK/FAILED→RETRY lifecycle.
- `compliance_api.py` — 9 endpoints. Full compliance HTTP surface.
- `bank_service.py` — disbursement lifecycle, Raast payout, reconciliation (~260 lines).
- `banking_api.py` — 11 endpoints. Full banking HTTP surface.
- `decision_api.py` — 7 endpoints + copilot. Wires DecisionEngine, PayrollGuardian, GovernanceService, HRCopilot. Includes `check_payroll_gate()`.
- `reporting_analytics_api.py` — InsightEngine wired. `get_anomaly_insights()` added.
- `whatsapp_service.py` — identity mapping, OTP, session management, inbound/outbound (~220 lines).
- `whatsapp_api.py` — 8 endpoints. Full WhatsApp HTTP surface.
- `docs/specs/mobile-layer.md` — MobileGatewayService documented.
- `integrations/whatsapp/webhook.py` — `attendance` command added (spec alignment fix).

## Session 3 — Spec alignment fixes + Pakistan audit (2026-04-12)

- **Spec alignment bugs fixed:**
  - `decision_api.py` `trigger_scan()` — wrong method name `detect_overtime_anomaly` corrected to `detect_overtime_spike(current_overtime_hours, historical_average_overtime_hours)`
  - `trigger_scan()` — only 2 of 4 guardian detectors called; added `detect_missing_tax()` and `detect_ghost_employee()`
  - `integrations/whatsapp/webhook.py` — `attendance` command missing from `_SUPPORTED_COMMANDS`; added with handler

- **Pakistan statutory audit — gaps G33–G42 registered (OPEN):**
  - G33 P0: filer/non-filer surcharge not applied (`statutory.py`)
  - G34 P0: tax slabs identical for 2024/2025/2026, no update path
  - G35 P0: exempt allowances not applied before slab lookup
  - G36 P1: EOBI contribution amounts not calculated
  - G37 P1: PESSI/SESSI contribution math missing
  - G38 P1: WPPF not implemented
  - G39 P1: WWF not implemented
  - G40 P2: `payroll_rules.py` generic — no Pakistan-specific rules
  - G41 P2: no slab update mechanism
  - G42 P2: ATL/FBR filer verification missing

- **Note:** `docs/specs/country/pakistan/compliance.md` already correctly specifies all of G33–G42. These are code-vs-spec mismatches only.

## Session 3 follow-up — Pakistan statutory G33–G42 implemented (2026-04-12)

All 10 statutory gaps registered in Session 3 are now DONE:

- `country/pakistan/statutory.py` — `calculate_tax()` extended with `tax_status` (non-filer surcharge) and `exemptions` (pre-slab deduction). `calculate_eobi()`, `calculate_social_security()`, `calculate_wppf()`, `calculate_wwf()`, `update_tax_slabs()` added.
- `country/pakistan/payroll_rules.py` — `apply_rules()` extended with statutory violations block (min wage PKR 32,000, overtime 2×, Eid bonus). Violations returned in `statutory_violations` list.
- `integrations/pakistan/atl_adapter.py` — new file. `verify_filer_status(cnic)` with 30-day cache. `bulk_verify()` and `clear_cache()` added. Safe fallback when ATL API unavailable.

## Session 4 — Canon Overlay Pass 3 (2026-04-12)

Read and overlaid 5 previously unread canon docs: `event-catalog.md`, `workflow-catalog.md`, `domain-model.md`, `data-architecture.md`, `read-models.md`. Found and closed 6 gaps (G43–G48):

- `event_contract.py` — 21 missing events added to `CANONICAL_EVENT_TYPES`: 7 travel, 8 compensation/benefits, 2 auth/engagement, 4 settings.
- `deployment/migrations/012_compensation_domain.sql` — new file (136 lines). Tables: `compensation_bands`, `salary_revisions`, `benefits_plans`, `benefits_enrollments`, `allowances`. Full PKs, FKs, constraints, indexes aligned to `data-architecture.md`.
- `deployment/migrations/013_travel_domain.sql` — new file (75 lines). Tables: `travel_requests`, `travel_itinerary_segments`. State machine enum, FKs, cascade delete, indexes aligned to `data-architecture.md`.

Canon overlay is now complete across all 13 canon docs. Gap register total: 48 gaps, 46 DONE, 1 DEFERRED (G27), 10 UI pages OPEN (G28).

## Session 5 — Behavior Spec Overlay, SB-G01–SB-G05 implemented (2026-04-12)

Read and overlaid `docs/system/HRMS SYSTEM BEHAVIOR SPEC.md` against code — assessed from an intent/capability angle (full capabilities always in core, expressed on demand). 5 gaps found and closed in one pass:

- **SB-G01** — `services/analytics/predictive.py`: all 3 predictive models now return `{model, prediction, confidence, explanation, supporting_data}` as mandated by spec §06 AI output contract. `supporting_data` contains per-factor `{value, weight, contribution}`; `explanation` is a human-readable narrative from top contributing factors. (NOTE 2026-06-13: `supporting_data` was renamed to `supporting_signals` under SPEC-G02 in Session 6, L209 of `progress.md` — this entry is left as the accurate historical record of the Session 5 state.)

- **SB-G02** — `attendance_service/service.py`: `punchStatus` field (`complete / missing_checkout / missing_checkin / no_punch`) added to every `_record_payload()` output. `detect_missing_punches(actor, from_date, to_date)` public surface method added — returns sorted list of incomplete punch records for payroll reconciliation.

- **SB-G03** — `error_registry.py` (new file at root): central error registry with 22 error codes across payroll, compliance, attendance, banking, WhatsApp, and general domains. Each entry: `{type, severity, resolution_steps, retryable}`. API: `get_error_descriptor(code)`, `register_error()`, `update_error()`. Fulfils spec §08 mandate that all errors be classified with explanation and resolution steps.

- **SB-G04** — `services/ai/payroll_guardian.py`: `_DEFAULT_THRESHOLDS` dict introduced. `__init__(self, thresholds=None)` added — accepts optional per-tenant overrides merged with defaults. All 4 detect methods now read thresholds from `self._thresholds`, making evidence strings and scoring dynamic. Spec §05 anomaly detection + §06 AI explainability both met.

- **SB-G05** — `integrations/whatsapp/webhook.py`: `CommandRegistry` class introduced with `register_command(keyword, intent)`, `resolve(keyword)`, `list_commands()`. Module-level `_command_registry` pre-registers 4 core P7 commands. `parse_intent()` uses registry. Error text lists commands dynamically from registry. Adapters extend without touching webhook.py.

Gap register total: 55 gaps, 52 DONE, 1 DEFERRED (G27), 10 UI pages OPEN (G28).

**Note:** Full QC suite has not been re-run since 2026-03-31 (pre-session 2). Tier 5 coverage gaps remain open for SB-G01–SB-G05 and session 2/3 services. `intent_build_alignment.md` alignment status is not updated until QC suite passes all tiers.
