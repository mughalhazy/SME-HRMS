...
> NOTE (Group C fix, 2026-06-13 — follow-up to normalisation pass 2026-06-08 / Cross-File Finding #15): `canon/api-standards.md` L8 ("All public routes are versioned under `/api/v1`") is the named routing authority. The `/api/manager/dashboard/...` paths below are non-compliant; the canonical form is `/api/v1/manager/dashboard/...`. Note also that the live implementation (`backend/api/manager_dashboard.py`, `ENDPOINTS`) registers only relative paths (`/alerts`, `/overtime`, `/approvals`, `/burnout`, `/performance`) without any `/api/v1/manager/dashboard` prefix — so this is a three-way divergence (spec doc, api-standards.md, and the actual route registration), not just a doc-vs-standard mismatch. Per [[feedback_divergence_resolution]] the paths below are left as-is (extension/annotation only); a future implementation pass should register `/api/v1/manager/dashboard/*` routes in the gateway to align all three.
