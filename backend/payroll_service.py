...
class PayrollService:
    ...
    def __init__(self, db_path: str | None = None, *, workflow_service: WorkflowService | None = None, notification_service: NotificationService | None = None, financial_wellness_service: FinancialWellnessService | None = None, leave_service: Any | None = None, country_resolver: CountryResolver | None = None):
        ...
        # SPEC-G06: leave → payroll effect. Optional injection; lazy import if not provided.
    ...
    def _run_guardian_precheck(self, records: list[dict[str, Any]]) -> dict[str, Any]:
        """Run PayrollGuardian anomaly checks before finalization.

        Checks each finalized record for missing tax deductions.
        Returns a confidence_signal and anomaly report.
        Critical anomalies (risk >= 70) must block finalization — caller is responsible for raising.
        Medium anomalies (risk 40-69) are surfaced as warnings, non-blocking.

        BEHAVIOR SPEC §02.1 — pre-run confidence signal.
        BEHAVIOR SPEC §02.2 — anomaly report before finalization.
        BEHAVIOR SPEC §02 (P1 fix, S8-G01).
        """
        ...
        for record in records:
            employee_id = str(record.get("employee_id", "unknown"))
            gross = float(record.get("gross_pay") or 0)
            deductions = float(record.get("deductions") or 0)

            # Check for missing tax deductions when gross exceeds the annual taxable threshold.
            # PKR 600,000 / year = PKR 50,000 / month minimum taxable (Finance Act 2024).
            # Uses a conservative 5% floor as expected minimum deduction for flagging.
            if gross > 50_000 and deductions == 0:
                result = guardian.detect_missing_tax(
                    expected_tax_amount=gross * 0.05,
        ...
        if critical_count > 0:
            confidence_signal = "issues_require_review"
        elif issues:
            confidence_signal = "warnings_found"
        else:
            confidence_signal = "safe_to_run"

        return {
            "confidence_signal": confidence_signal,
            "employees_checked": len(records),
    ...
    def _build_record_from_payload(
        ...
        *,
        ...
        # SPEC-G06: leave → payroll effect — deduct approved unpaid leave days
        unpaid_leave_deduction = Decimal("0.00")
        if employee_id is not None and self._leave_service is not None:
            try:
                unpaid_days = self._leave_service.get_unpaid_leave_days(
                    str(employee_id),
                    pay_period_start.isoformat(),
                    pay_period_end.isoformat(),
                )
                if unpaid_days > 0:
                    # Daily rate = gross_salary / calendar working days in period (default 26/month)
                    period_days = max(1, (pay_period_end - pay_period_start).days + 1)
                    daily_rate = (base_salary + allowances + overtime_pay) / Decimal(str(period_days))
                    unpaid_leave_deduction = (daily_rate * Decimal(str(unpaid_days))).quantize(Decimal("0.01"))
            except Exception:
                pass  # Leave service unavailable — do not block payroll

        deductions = (deductions + component_deductions + rule_deductions + tax_deduction + salary_advance_deduction + unpaid_leave_deduction).quantize(Decimal("0.01"))
    ...
    def run_payroll(self, period_start: str, period_end: str, authorization: str | None, records: list[dict[str, Any]] | None = None, idempotency_key: str | None = None, trace_id: str | None = None) -> tuple[int, dict[str, Any]]:
        ...
        try:
            ...
            with self._lock, self._transaction():
                ...
                # BEHAVIOR SPEC §02 — guardian pre-run check before compliance precheck.
                guardian_report = self._run_guardian_precheck(finalized_records)
                if guardian_report["critical_count"] > 0:
                    raise ServiceError(
                        "PAYROLL_GUARDIAN_BLOCKED",
                        f"PayrollGuardian detected {guardian_report['critical_count']} critical anomaly(ies). "
                        "Review and resolve before finalizing payroll.",
                        422,
                        details=guardian_report["issues"],
                    )
