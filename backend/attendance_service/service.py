...
class AttendanceService:
    ...
    @staticmethod
    def _punch_status(record: AttendanceRecord) -> str:
        """Compute punch completeness at record payload time."""
    ...
    def _record_payload(record: AttendanceRecord) -> dict:
        return {
            ...
            'punchStatus': (
                'complete' if (record.check_in_time and record.check_out_time) else
                'missing_checkout' if record.check_in_time else
                'missing_checkin' if record.check_out_time else
                'no_punch'
            ),
    ...
    def detect_missing_punches(
        ...
        *,
        ...
        """Return all attendance records with incomplete punch pairs in the date range."""
        if actor.role not in {'Admin', 'Manager'}:
            raise AttendanceServiceError('FORBIDDEN', 'Only Admin or Manager can run missing punch detection')
        if to_date < from_date:
            raise AttendanceServiceError('DATE_RANGE_INVALID', 'to must be >= from')
        results = []
        for record in self._records.values():
            if not (from_date <= record.attendance_date <= to_date):
                continue
            status = self._punch_status(record)
            if status != 'complete':
                results.append({
                    'attendanceId': str(record.attendance_id),
                    'employeeId': str(record.employee_id),
        ...
        return sorted(results, key=lambda x: (x['attendanceDate'], x['employeeId']))
