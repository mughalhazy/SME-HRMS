...
class LeaveService:
    ...
    def get_unpaid_leave_days(
        ...
        """Return total approved UNPAID leave days for an employee within a pay period.

        Called by payroll_service.run_payroll() to compute leave deductions.
        Only APPROVED unpaid leave requests that overlap with the pay period are counted.

        Args:
            employee_id: employee to query
            period_start: pay period start date as "YYYY-MM-DD"
            period_end:   pay period end date as "YYYY-MM-DD"
            tenant_id:    optional tenant scope

        Returns:
            Total unpaid leave days (float) to deduct from gross pay. 0.0 if none.
        """
        try:
        ...
        _tenant = normalize_tenant_id(tenant_id)
        total_unpaid_days: float = 0.0
        ...
        with self._lock:
            for req in self.requests.values():
                ...
                if req.status != LeaveStatus.APPROVED:
                    continue
                if _tenant and req.tenant_id != _tenant:
                    continue
                # Overlap check: request must overlap with the pay period
                overlap_start = max(req.start_date, p_start)
                overlap_end = min(req.end_date, p_end)
                if overlap_start <= overlap_end:
                    overlap_days = (overlap_end - overlap_start).days + 1
                    # Scale by partial_day_portion if applicable
                    total_unpaid_days += overlap_days * req.partial_day_portion
