...
def submit_contribution_return(
    ...
    *,
    ...
    track.create_pending(submission_id, "PESSI_SESSI", "C-1", payload)

    if errors:
        result = {"status": "failed", "submitted": False, "submission_status": "failed", "submission_id": submission_id, "provider": "PESSI_SESSI", "format": "C-1", "errors": errors, "response_payload": {"errors": errors}}
    ...
    for attempt in range(1, max(attempts, 1) + 1):
        try:
            ...
            if not body.get("ack_id"):
                ...
                result = {"status": "failed", "submitted": False, "submission_status": "failed", "submission_id": submission_id, "provider": "PESSI_SESSI", "format": "C-1", "errors": errors, "response_payload": body}
                track.mark_failed(submission_id, body)
                return result

            track.mark_submitted(submission_id, body)
            track.mark_acknowledged(submission_id, body)
            return {"status": "submitted", "submitted": True, "submission_status": "submitted", "submission_id": submission_id, "provider": "PESSI_SESSI", "format": "C-1", "errors": [], "ack_id": str(body["ack_id"]), "response_payload": body}
            ...
            if attempt < max(attempts, 1):
                ...
                continue
            return {"status": "failed", "submitted": False, "submission_status": "failed", "submission_id": submission_id, "provider": "PESSI_SESSI", "format": "C-1", "errors": [f"{exc.code}: {exc.message}"], "http_status": exc.status_code, "response_payload": response_payload}
    return {"status": "failed", "submitted": False, "submission_status": "failed", "submission_id": submission_id, "provider": "PESSI_SESSI", "format": "C-1", "errors": ["Unexpected retry termination"], "response_payload": {}}
