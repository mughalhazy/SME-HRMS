"""
Payroll Computation Engine — services/payroll_service.py
=========================================================
This is the **pure computation layer** for payroll. It handles the math:
gross/taxable/net calculation, overtime, shift pay, penalties, policy rules.

This is NOT the payroll HTTP service. The HTTP surface lives in:
  - payroll_api.py          — API handlers
  - payroll_service.py      — full PayrollService with auth, observability, persistence (root)

This module's PayrollService is consumed by the root payroll_service.py internally
as the computation delegate. It has no HTTP surface and no DB access of its own.

Architecture rule: all country-specific tax computation is delegated via
CountryResolver → adapter.tax_engine.calculate_tax(). No Pakistan-specific import here.
"""
