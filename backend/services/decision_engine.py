...
class DecisionCard:
    ...
    reversibility: str
    expires_at: str
    # SPEC §15 required fields
    source_domain: str = "system"          # domain that triggered this card (payroll/attendance/compliance/ai)
    severity: DecisionSeverity = DecisionSeverity.NOTIFY  # drives blocking behaviour
    resolved_at: str | None = None         # ISO timestamp when decision was resolved
    actor: str | None = None               # who resolved the decision
    # Lifecycle / internal
    status: str = "active"
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "severity": self.severity.value if isinstance(self.severity, DecisionSeverity) else self.severity,
            "impact": self.impact,
...
class DecisionEngine:
    ...
    def generate_from_anomalies(self, anomalies: list[dict[str, Any]]) -> list[DecisionCard]:
        ...
        for anomaly in anomalies:
            ...
            # Map risk_score to spec §15 severity levels
            if risk_score >= 70:
                sev = DecisionSeverity.CRITICAL
            elif risk_score >= 40:
                sev = DecisionSeverity.NOTIFY
            else:
                sev = DecisionSeverity.PASSIVE
    ...
    def generate_from_compliance_issues(self, issues: list[dict[str, Any]]) -> list[DecisionCard]:
        ...
        for issue in issues:
            ...
            sev = DecisionSeverity.CRITICAL if severity == "high" else (
                DecisionSeverity.NOTIFY if severity == "medium" else DecisionSeverity.PASSIVE
            )
            cards.append(
                self.create_card(
    ...
    def create_card(
        ...
        severity: DecisionSeverity | str = DecisionSeverity.NOTIFY,
    ) -> DecisionCard:
        now = datetime.now(timezone.utc)
        expires_at = (now + timedelta(hours=self.default_ttl_hours)).isoformat()
        _severity = severity if isinstance(severity, DecisionSeverity) else DecisionSeverity(severity)
        card = DecisionCard(
            trigger=trigger,
            impact=impact,
            confidence=confidence,
    ...
    def resolve_card(self, card: DecisionCard, *, actor: str, reason: str = "resolved") -> DecisionCard:
        """Mark a decision card as resolved — sets resolved_at and actor per SPEC §15."""
        if card.status == "expired":
            raise ValueError("Cannot resolve an expired decision card")
        now = datetime.now(timezone.utc).isoformat()
        card.status = "resolved"
        card.lifecycle_state = "resolve"
