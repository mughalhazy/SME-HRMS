...
class FinancialWellnessService:
    ...
    def request_salary_advance(self, *, employee_id: str, amount: float, currency: str = "PKR") -> dict[str, object]:
        ...
        emit_canonical_event(
            _outbox,
            legacy_event_name="AdvanceRequested",
            data={"request_id": request.request_id, "employee_id": request.employee_id,
                  "amount": str(request.amount), "currency": request.currency},
            source="ewa-financial-service",
    ...
    def approve_salary_advance(self, *, request_id: str, approver_id: str) -> dict[str, object]:
        ...
        emit_canonical_event(
            _outbox,
            legacy_event_name="AdvanceApproved",
            data={"request_id": request_id, "employee_id": request.employee_id,
                  "approved_by": approver_id, "amount": str(request.amount)},
            source="ewa-financial-service",
            tenant_id=DEFAULT_TENANT_ID,
            registry=_registry,
        )
