...
class DataIntegrityValidator:
    ...
    def register_background_jobs(self, background_jobs: BackgroundJobService) -> None:
        ...
        background_jobs.register_handler('integrity.repair', handler, max_attempts=1, timeout_seconds=10.0)
    ...
    def _validate_projection_integrity(self) -> list[IntegrityIssue]:
        ...
        if self.search_service is not None:
            ...
            rebuilt = SearchIndexingService(db_path=f'file:shadow-{uuid.uuid4().hex}?mode=memory')
        ...
        if self.reporting_service is not None:
            ...
            shadow = ReportingAnalyticsService(db_path=f'file:shadow-{uuid.uuid4().hex}?mode=memory', tenant_id=self.tenant_id)
